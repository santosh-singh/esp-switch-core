        ...
        const String& httpsFingerprint)
{
    ...
    DEBUG_HTTP_UPDATE("[httpUpdate-update] URL: %s version: %s\n", url.c_str(), currentVersion.c_str());
    return handleUpdate(http, currentVersion, false);
}
...
HTTPUpdateResult ESP8266HTTPUpdate::updateSpiffs(const String& url, const String& currentVersion, const String& httpsFingerprint)
{
    HTTPClient http;
    http.begin(url, httpsFingerprint);
    DEBUG_HTTP_UPDATE("[httpUpdate-updateSpiffs] URL: %s version: %s\n", url.c_str(), currentVersion.c_str());
...
HTTPUpdateResult ESP8266HTTPUpdate::handleUpdate(HTTPClient& http, const String& currentVersion, bool spiffs)
{
    ...
    http.addHeader(F("Authorization"), "Bearer "+String(accessToken));
    http.addHeader(F("deviceUUID"), String(deviceUUID));
    ...
    const char * headerkeys[] = { "x-MD5", "x-ESP8266-VERSION" };
    ...
    case HTTP_CODE_OK:  ///< OK (Start Update)
        DEBUG_HTTP_UPDATE("[httpUpdate]  - x-MD5 hash: %s\n", http.header("x-MD5").c_str());
        DEBUG_HTTP_UPDATE("[httpUpdate]  - x-ESP8266-VERSION version: %s\n", http.header("x-ESP8266-VERSION").c_str());
        if(len > 0) {
            ...
            if(!startUpdate) {
                ...
                if(runUpdate(*tcp, len, http.header("x-MD5"), command)) {
                    ...
                    if (http.hasHeader("x-ESP8266-VERSION")){
                      strcpy(newVersion, http.header("x-ESP8266-VERSION").c_str());
                      DEBUG_HTTP_UPDATE("[httpUpdate] new version received: %s\n", newVersion);
                    }else{
                      DEBUG_HTTP_UPDATE("[httpUpdate] version header not received\n");
                    }
                    http.end();

                    if(_rebootOnUpdate) {
                        DEBUG_HTTP_UPDATE("[httpUpdate] now restarting\n");
                        ESP.restart();
                    }
